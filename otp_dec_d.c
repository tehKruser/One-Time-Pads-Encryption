...
void decryptInputString(char* inputString, char* keyString);
...
void forkChild(int establishedConnectionFD){
    ...
    switch (spawnPid) {
        ...
        case 0: {   // child process
            ...
            if(strcmp(token, "otp_dec") != 0){
                charsRead = send(establishedConnectionFD, "Connection refused", 18, 0); // Send success back
                if (charsRead < 0) error("ERROR writing to socket");
            } else {
                token = strtok(NULL, "#");
                strcpy(decryptString, token);

                token = strtok(NULL, "@");
                strcpy(keyString, token);

                // decrypt string
                decryptInputString(decryptString, keyString);

                // Send a Success message back to the client
                charsRead = send(establishedConnectionFD, decryptString, strlen(decryptString), 0); // Send success back
                if (charsRead < 0) error("ERROR writing to socket");
            }
